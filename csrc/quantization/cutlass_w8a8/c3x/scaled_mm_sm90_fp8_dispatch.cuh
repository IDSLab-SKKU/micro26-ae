...
/**
 * Gemm kernel configuration for SM90 (fp8).
 *
 * Batch-invariant by construction: a single kernel config serves every M, so a
 * given output element is bit-identical regardless of how many tokens are
 * processed together. The config uses cutlass::gemm::PersistentScheduler (no
 * split-K), so a fixed TileShape fixes the per-output-element K-reduction order
 * across batch sizes.
 *
 * Selecting the tile shape by M, as a throughput-tuned dispatcher does, makes
 * that reduction order a function of batch size and the output varies with it.
 * Emulation results are compared against this path across architectures, which
 * requires the batch-size degree of freedom to be removed.
 */
...
// Single batch-invariant config used for ALL M.
...
inline void cutlass_gemm_sm90_fp8_dispatch(torch::Tensor& out,
  ...
  // Batch-invariant: one config for all M; normal (non-swapped) scale order.
